...
void QueueEnqueue(Queue q, Item it) {
    ...
    int length = q->frontIndex + q->size;
...
static void increaseCapacity(Queue q) {
    ...
    Item *temp = malloc((q->capacity*2) * sizeof(Item));
    if (temp == NULL) {
    ...
    for (int i = 0; i < q->capacity; i++) {
        ...
        int length = i;
        if (length > q->capacity) length -= q->capacity - 1;
        
        temp[i] = q->items[length];
...
Item QueueDequeue(Queue q) {
    ...
    Item temp = q->items[q->frontIndex];
